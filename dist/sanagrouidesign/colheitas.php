    ...
    <div id="wrapper">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <div class="d-sm-flex align-items-center justify-content-between mb-4">
                        <h1 class="h3 mb-0 text-gray-800">Minhas Colheitas</h1>
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content">
                                    <div class="modal-header">
                                        <h5 class="modal-title" id="exampleModalLabel">Nova Colheita</h5>
                                        <button type="button" class="btn-close" data-bs-dismiss="modal"
                                            aria-label="Close"></button>
                                    </div>
                                    <div class="modal-body">
                                        <form class="row g-3" action="php/cadcolheitas.php" method="POST">
                                            <?php
                                                $iduser = $_SESSION['id'];
                                                $sql = "select id_crop, name from crop where id='$iduser' and cro_status='Em andamento'";
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <select name="tSafra" class="form-select"
                                                    aria-label="Default select example">
                                                    <option selected>Escolha...</option>
                                                    <?php while($row = mysqli_fetch_assoc($query)){?>
                                                    <option value="<?php echo $row['id_crop'] ?>">
                                            ...
                                            <div class="col-md-7">
                                                <label for="exampleFormControlInput1"
                                                    class="form-label">Produção</label>
                                                <input type="text" name="tProducao" input class="form-control"
                                                    id="producao" placeholder="Exemplo: 1.500Kg">
                                            </div>
                                            <div class="col-md-5">
                                                <label for="inputState" class="form-label">Unidade de Medida</label>
                                                <select id="inputState" id="medida" name="tUnit" class="form-select">
                                                    <option selected>Escolha...</option>
                    ...
                    <?php
                            ...
                            $id = $_SESSION['id'];
                            $sql2 = "select a.id_crop, id_harvest, name, cultive, production, dateend, metric, c.created_at
                            from crop a, harvest c
                            where c.id_crop = a.id_crop and c.id='$iduser'                            
                                    ...
                                    <div class="row no-gutters align-items-center">
                                        <div class="col mr-2">
                                            ...
                                            <div class="h5 mb-0 font-weight-bold text-gray-800" style="color: limegreen; font-size: 15px;">
                                                <?php echo $row['production']?> <?php echo $row['metric']?></div>
                                        </div>
                                        <div class="col-auto">
                                        <a href="#editArea" class="btn btn-circle" style="color: limegreen;"
                                                        data-bs-toggle="modal" data-bs-target="#editarea"
                                                        data-bs-whateverid="<?php echo $row['id_harvest'];?>"
                                                        data-bs-whateversafra="<?php echo $row['id_crop'];?>"    
                                                        data-bs-whatevermetrica="<?php echo $row['metric'];?>"
                                                        data-bs-whateverproducao="<?php echo $row['production'];?>"                                                                                                                
                                                        data-bs-whateverfim="<?php echo $row['dateend'];?>">
                                                        <i class="far fa-edit"></i>
                                                    </a>


                                                    <?php echo
                                                            "<style>button{background-color:transparent;color:white;border-style:none; border-radius:50px; padding: 5px;}.buttons{text-decoration:none;color:red;}.buttons:hover{color: darkred;}.edit{background-color:limegreen;}</style><button><a class='buttons' href='php/deletecolheita.php?id=".$row['id_harvest']."'><i class='far fa-trash-alt'></i></a></button>";  
                                                        ?>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <?php } 
                            ...
                            $id = $_SESSION['id'];
                            $result_pg = "SELECT COUNT(id_harvest) AS num_result FROM harvest where id='$id'";
                            ...
                            if ($row_pg['num_result'] == 0){
                                echo "<h1 class='h3 mb-0 text-gray'>Você não possui nenhuma Colheita cadastrada!</h1><br>";                                
                                
                            } else {
                                //Limitar os link antes depois
                            $max_links = 2;
                            echo "<style>a { text-decoration: none; color: inherit; }a:hover{text-decoration: none; color: limegreen;}</style><a class='link' href='colheitas.php?pagina=1'>Primeira</a> ";
                           
                            for($pag_ant = $pagina - $max_links; $pag_ant <= $pagina - 1; $pag_ant++)
                            {
                                if($pag_ant >= 1)
                                {
                                    echo "<a href='colheitas.php?pagina=$pag_ant'>$pag_ant</a> ";
                                }
                            }
                            ...
                            for($pag_dep = $pagina + 1; $pag_dep <= $pagina + $max_links; $pag_dep++)
                            {
                                ...
                                {
                                    echo "<a href='colheitas.php?pagina=$pag_dep'>$pag_dep</a> ";
                                }
                            }
                           
                            echo "<a href='colheitas.php?pagina=$quantidade_pg'>Ultima</a>";
                            }
                        ?>
                    <!-- End of Main Content -->
            ...
            <div class="modal fade" id="editarea" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <form class="row g-3" action="php/editcolheita.php" method="POST">
                                <?php
                                                $iduser = $_SESSION['id'];
                                                $sql = "select id_crop, name from crop where id='$iduser' and cro_status='Finalizada'";
                                                $query = mysqli_query($conn, $sql);
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="hidden" class="form-control" name="tUser"
                                        value="<?php echo $_SESSION['id']; ?>">
                                    <select name="tSafra" class="form-select" id="safra" aria-label="Default select example">
                                        <option selected>Escolha...</option>
                                        <?php while($row = mysqli_fetch_assoc($query)){?>
                                        <option value="<?php echo $row['id_crop'] ?>">
                                            <?php echo $row['name']?></option>
                                ...
                                <div class="col-md-7">
                                    <label for="exampleFormControlInput1" class="form-label">Produção</label>
                                    <input type="number" name="tProducao" input class="form-control" id="producao"
                                        placeholder="Exemplo: 1.500Kg">
                                </div>
                                <div class="col-md-5">
                                                <label for="inputState" class="form-label">Unidade de Medida</label>
                                                <select id="medida" name="tUnit" class="form-select">
                                                    <option selected>Escolha...</option>
                                                    <!--<option value="Kg">Kg</option>-->
                                                    <option value="toneladas">toneladas</option>
                                                </select>
                                ...
                                <div class="col-md-7">
                                    <label for="exampleFormControlInput1" class="form-label">Data Final da Colheita</label>
                                    <input type="date" id="fim" name="tDtend" style="border: grey; outline: none; color: grey;"
                                        class="form-date">
                                </div>
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-secondary"
            ...
            exampleModal.addEventListener('show.bs.modal', function(event) {
                ...
                var recipientsafra = button.getAttribute('data-bs-whateversafra')  
                var recipientnome = button.getAttribute('data-bs-whateverproducao')                
                var recipientfim = button.getAttribute('data-bs-whateverfim')
                var recipientmetric = button.getAttribute('data-bs-whatevermetrica')
                ...
                var nome = exampleModal.querySelector('#producao')
                var fim = exampleModal.querySelector('#fim')
                var safra = exampleModal.querySelector('#safra')
                var metrica = exampleModal.querySelector('#medida')


                id.value = recipient
                nome.value = recipientnome                
                fim.value = recipientfim
                safra.value = recipientsafra
                metrica.value = recipientmetric
            })
            </script>
